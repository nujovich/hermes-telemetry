...
def _build_pricing_yaml(models: dict[str, dict]) -> str:
    """Build a pricing.yaml string from a seed dict (manual overrides only).

    v0.6 schema: the seed lives under ``overrides."*"`` (provider-neutral).
    Auto-fetched entries go to pricing.auto.yaml, written separately by
    ``_write_auto_pricing``.
    """
    ...
    header = (
        "# ~/.hermes/telemetry/pricing.yaml — MANUAL pricing overrides.\n"
        "# Auto-generated by hermes-telemetry setup. Hand-edit freely.\n"
        "# Prices in USD per 1 million tokens.\n"
        "#\n"
        "# Schema (v0.6+):\n"
        "#   overrides:\n"
        "#     <provider-name>:        # 'nous', 'openrouter', 'nvidia', ...\n"
        '#       "model-id":\n'
        "#         input: 1.00\n"
        "#         output: 3.00\n"
        ...
        "#         output: 1.50\n"
        "#\n"
        "# Auto-fetched prices live in pricing.auto.yaml (machine-managed).\n"
        "#\n"
    )
    return header + _dump_yaml(data)


def _build_auto_pricing_yaml(sources: dict[str, dict[str, dict]]) -> str:
    """Build a pricing.auto.yaml string from per-source model dicts."""
    data: dict = {"sources": sources}
    header = (
        "# ~/.hermes/telemetry/pricing.auto.yaml — AUTO-FETCHED pricing.\n"
        "# Machine-managed. Edit pricing.yaml for overrides instead — anything\n"
        "# you put here will be overwritten on the next refresh.\n"
        "#\n"
    )
    return header + _dump_yaml(data)
...
def _write_auto_pricing(sources: dict[str, dict[str, dict]]) -> Path:
    _tele_dir().mkdir(parents=True, exist_ok=True)
    content = _build_auto_pricing_yaml(sources)
    p = _tele_dir() / "pricing.auto.yaml"
    p.write_text(content)
    return p
...
def handle_command(raw_args: str) -> str:
    ...
    if sub == "pricing":
        ...
        choice = parts[1]
        if choice == "auto":
            # v0.6: built-in seed goes to pricing.yaml (manual overrides),
            # OpenRouter fetch goes to pricing.auto.yaml (machine-managed).
            seed = dict(_DEFAULT_SEED)
            or_models = _fetch_openrouter_models()
            path = _write_pricing(seed)
            auto_path = _write_auto_pricing({"openrouter": or_models}) if or_models else None
            _reload_pricing_cache()
            tail = f" + {len(or_models)} from OpenRouter → {auto_path}" if auto_path else ""
            return (
                f"Pricing configured: {len(seed)} built-in models written to {path}{tail}\n"
