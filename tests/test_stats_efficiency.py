...
def test_efficiency_runs_penalizes_failures():
    """Failed sessions should receive a heavy penalty."""
    now = db._utcnow()
    db.start_run("s1", model="m", platform="cli")
    db.record_llm_call("s1", now, "modelA", "openai", 100, 100, 0.001, 50)
    db.end_run("s1", "failed")

    rows = db.efficiency_runs(window_hours=24)
    assert len(rows) == 1
    r = rows[0]
    # output_ratio = 1.0, contribution = 40
    # error_penalty = 30 (failed), turn_penalty = 1.5
...
def test_efficiency_runs_cancelled_penalty():
    """Cancelled sessions get a smaller penalty than failed ones."""
    now = db._utcnow()
    db.start_run("s1", model="m", platform="cli")
    db.record_llm_call("s1", now, "modelA", "openai", 100, 100, 0.001, 50)
    db.end_run("s1", "cancelled")

    rows = db.efficiency_runs(window_hours=24)
    assert len(rows) == 1
    # error_penalty = 15 (cancelled)
    # score = 40 + 40 - 15 - 1.5 = 63.5
    assert rows[0]["efficiency_score"] == 63.5
